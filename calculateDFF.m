function dff = calculateDFF(roiFluorescences)
    fs = 5;       % sample rate, Hz
    ...
    for i=1:1:size(roiFluorescences,2)
        for j=1:1:size(roiFluorescences,1)
            ...
            if j-int32(win_samples/2) < 1
                f0 = prctile(roiFluorescences(1:j+int32(win_samples/2),i),5);
            elseif j+int32(win_samples/2) > size(roiFluorescences,1)
                f0 = prctile(roiFluorescences(j-int32(win_samples/2):size(roiFluorescences,1)-1,i),5);
            else
                f0 = prctile(roiFluorescences(j-int32(win_samples/2):j+int32(win_samples/2),i),5);
            end
